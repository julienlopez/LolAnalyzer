...
{
    return tl::make_unexpected(U("unsupported type"));
}

template <class T> inline Result<std::vector<T>> parseArray(const web::json::value& value)
{
    if(not value.is_array()) return tl::make_unexpected(U("not an array"));
    std::vector<T> res;
...
template <> Result<std::size_t> inline cast<std::size_t>(const web::json::value& value)
{
    if(value.is_number())
        ...
        return tl::make_unexpected(U("not the right type : number"));
}
...
Result<std::tuple<TYPE1, TYPES...>> parseSimpleObject(const web::json::object& object, const utility::string_t& name1,
                                                      STRS... names)
{
...
template <class TYPE1, class... TYPES, class... STRS>
Result<std::tuple<TYPE1, TYPES...>> parseSimpleObject(const web::json::value& json, const utility::string_t& name1,
                                                      STRS... names)
{
    if(json.is_object())
        return parseSimpleObject<TYPE1, TYPES...>(json.as_object(), name1, names...);
    else
        return tl::make_unexpected(U("json is not an object"));
}
